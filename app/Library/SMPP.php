...
class SMPP
{
     ...
     */
    function send_pdu($id, $data): bool|array|string
    {
        if (!$this->socket || !is_resource($this->socket)) {
            return "Send PDU: Invalid or closed socket!";
        }
        ...
        // send PDU
        // $this.->smppClient($this->socket, $pdu);
        ...
        // Get response
        $data = stream_get_contents($this->socket, $command_length - 4);
     ...
     * @return bool|string
     */
    function open($host, $port, $system_id, $password): bool|string
    {
        $this->socket = @fsockopen($host, $port, $errno, $err_str, $this->timeout);
    
        if (!$this->socket) {
            return "Failed to open socket: $err_str ($errno)";
        }
     ...
     * @return bool
     */
    function submit_sm($source_addr, $destination_addr, $short_message, string $optional = ''): bool
    {
     ...
     * @return bool
     */
    function send_long($source_addr, $destination_addr, $short_message, int $utf = 0, int $flash = 0): bool
    {
