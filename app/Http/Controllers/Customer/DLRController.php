...
class DLRController extends Controller
{
     ...
     * @throws Throwable
     */
    public static function inboundDLR($to, $message, $sending_sever, $cost, $from, $media_url = null, $extra = null, int $user_id = 1): JsonResponse|string
    {
        ...
        // dd($to, $message, $sending_sever, $cost, $from );
        $to   = str_replace(['(', ')', '+', '-', ' '], '', trim($to));
        $from = ($from != null) ? str_replace(['(', ')', '+', '-', ' '], '', trim($from)) : null;

        $phoneNumberUtil   = PhoneNumberUtil::getInstance();
        $phoneNumberObject = $phoneNumberUtil->parse('+'.$from);
...
// 
        if (strlen($to)<=7) {
            $sender_id = Senderid::where('sender_id', $to)->first();
        
            if ($sender_id) {
                $user_id = $sender_id->user_id;
                $user    = User::find($user_id);

                // dd($user->is_customer  && $user->sms_unit != '-1' );
                if ($user->is_customer ) {
                ...
                if ($chatBox) {
                    ...
                    // dd( $sender_id->sender_id);
                    // dd($chatBox->id,$message,$media_url,$sms_type,$sms_type,6);
            ...
            $phone_number = PhoneNumbers::where('number', $to)
                                        ->where('status', 'assigned')
                                        ->first();

            if ( ! $phone_number) {
                $phone_number = PhoneNumbers::where('number', 'like', "%{$to}%")
                                            ->where('status', 'assigned')
                                            ->first();
            }
    ...
    public function inboundEasySendSMS(Request $request): JsonResponse|string
    {
        ...
        // dd($request);
        if ($message == '' || $to == ''|| $from == '') {
            return 'To , From and Message value are required';
        }
        ...
        // dd($to, $message, 'FlowRoute', $cost, $from);
        // dd($message_count,$cost);
        return $this::inboundDLR($to, $message, 'FlowRoute', $cost, $from);
    }
