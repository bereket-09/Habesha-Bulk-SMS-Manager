...
@section('title', __('locale.menu.Trivia Builder'))
...
<!-- 
                                    ...
                                    <!-- This is were we build the questions. -->
<!-- 
                                    <div class="col-12">
                                        <div class="mb-1">
    ...
    <div class="row">
        <div class="col-md-12 " style="background:#ECECEC;">
            <div class="card" style="margin-top:15px;border-radius: 10px;">
            <div class="card-header" style="display:flex;justify-content:center; font-weight: bold; font-size: 1.1rem;">{{ __('Create Questions for the Trivia') }}</div>
                ...
                <div class="card-body">
                    <form method="POST" action="#">
                        ...
                        <div id="questions-container">
                            ...
                            <div class="question-group mb-4">
                                <div class="d-flex align-items-center justify-content-between" style="gap:15px;">
                                    ...
                                    <textarea class="form-control question-input ml-1 mr-2" name="questions[]" rows="2" style="width: 100%;" required placeholder="Write your question here"></textarea>
                                    <button type="button" class="btn btn-danger btn-remove-question" title="Remove Question">&times;</button>
                                </div>

                                <hr> <!-- Horizontal line after each question -->

                                <!-- Options and Answers -->
                                <div class="options-container2">

                                <div class="options-container mt-2 d-flex" style="gap:15px;">
                                    <div class="option-group mb-1" style="flex: 6;">
                                        <input type="text" class="form-control option-input" name="options[0][]" placeholder="Option text here" style="width: 100%;" required>
                                    </div>
                                    <div class="answer-group mb-1" style="flex: 2.5;">
                                        <input type="text" class="form-control answer-input" name="answers[0][]" placeholder="Answer to expect" style="width: 100%;" required>
                                    </div>
                                    <div class="radio-group mb-1" >
                                        <!-- <input type="radio" class="form-check-input correct-answer-radio" name="correct_answers[0]" value="0" required selected>  -->
                                        <input type="radio" class="form-check-input correct-answer-radio" name="correct_answers[0]" value="${optionIndex}" required checked>

                                    </div>
                                    <div class="remove-btn-group mb-1" style="flex: 1;">
                                        <button type="button" class="btn btn-danger btn-remove-option" title="Remove Option" style="width: 100%;">&times;</button>
                                    </div>
                                </div>
                                
                                </div>
                                
                                <button type="button" class="btn btn-primary btn-add-option" title="Add Option">{{ __('Add Option') }}</button>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            <button type="button" class="btn btn-success btn-add-question">{{ __('Add Question') }}</button>
                        </div>
...
<script>
    ...
    document.querySelector('.btn-add-question').addEventListener('click', function() {
        ...
        newQuestionGroup.innerHTML = `
            <hr>     <hr>
                                ...
                                <div class="d-flex align-items-center justify-content-between" style="gap:15px;">
                                    <div class="question-count">${questionCount}</div>
                                    <textarea class="form-control question-input ml-1 mr-2" name="questions[]" rows="2" style="width: 100%;" required placeholder="Write your question here"></textarea>
                                    <button type="button" class="btn btn-danger btn-remove-question" title="Remove Question">&times;</button>
                                </div>
                                ...
                                <div class="options-container mt-2 d-flex" style="gap:15px;">
                                    <div class="option-group mb-1" style="flex: 6;">
                                        <input type="text" class="form-control option-input" name="options[${questionCount - 1}][]" placeholder="Option text here" style="width: 100%;" required>
                                    </div>
                                    <div class="answer-group mb-1" style="flex: 2.5;">
                                        <input type="text" class="form-control answer-input" name="options[${questionCount - 1}][]" placeholder="Answer to expect" style="width: 100%;" required>
                                    </div>
                                    <div class="radio-group mb-1">
                                        <input type="radio" class="form-check-input correct-answer-radio" name="correct_answers[${questionCount - 1}]" value="0" required checked>
                                    </div>
                                    <div class="remove-btn-group mb-1" style="flex: 1;">
                                        <button type="button" class="btn btn-danger btn-remove-option" title="Remove Option" style="width: 100%;">&times;</button>
                                    </div>
    ...
    document.addEventListener('click', function(e) {
        if (e.target && e.target.classList.contains('btn-remove-option')) {
            e.target.closest('.options-container').removeChild(e.target.closest('.option-group'));
        }
    });
    ...
    function resetQuestionCounter() {
        ...
        questionCount = 1;
        questionGroups.forEach(function(group, index) {
            group.querySelector('.question-count').textContent = questionCount++;
        });
    }
// Add Option Dynamically
document.addEventListener('click', function(e) {
    if (e.target && e.target.classList.contains('btn-add-option')) {
        let questionGroup = e.target.closest('.question-group');
        let questionCount = Array.from(questionGroup.parentElement.children).indexOf(questionGroup) + 1;
        
        let optionsContainer = questionGroup.querySelector('.options-container2');
        let newOptionGroup = document.createElement('div');
        newOptionGroup.classList.add('options-container', 'mt-2', 'd-flex');
        newOptionGroup.style.gap = '15px';
        newOptionGroup.innerHTML = `
            <div class="option-group mb-1" style="flex: 6;">
                <input type="text" class="form-control option-input" name="options[${questionCount - 1}][]" placeholder="Option text here" style="width: 100%;" required>
            </div>
            <div class="answer-group mb-1" style="flex: 2.5;">
                <input type="text" class="form-control answer-input" name="answers[${questionCount - 1}][]" placeholder="Answer to expect" style="width: 100%;" required>
            </div>
            <div class="radio-group mb-1" >
                <input type="radio" class="form-check-input correct-answer-radio" name="correct_answers[${questionCount - 1}]" value="0" required>
            </div>
            <div class="remove-btn-group mb-1" style="flex: 1;">
                <button type="button" class="btn btn-danger btn-remove-option" title="Remove Option" style="width: 100%;">&times;</button>
            </div>
