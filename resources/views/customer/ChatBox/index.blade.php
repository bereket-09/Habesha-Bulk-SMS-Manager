...
function fetchUpdatedMessages() {
    let chatId = $(".chat_id").val(); // Get the chat ID of the opened chat box

    // Send AJAX request to fetch updated messages for the opened chat box
    $.ajax({
        url: `{{ url('/chat-box') }}/${chatId}/messages`,
        type: "GET",
        ...
        success: function (response) {
            ...
            // Iterate through new messages and append them to the chat history
            cwData.forEach((sms) => {
                let mediaUrl = '';
                if (sms.media_url !== null) {
                    ...
                    if (fileType === 'video') {
                        mediaUrl = `<p><video src="${sms.media_url}" controls>Your browser does not support the video tag. <video/></p>`;
                    } else if (fileType === 'audio') {
                        mediaUrl = `<p><audio src="${sms.media_url}" controls>Your browser does not support the audio element. </audio></p>`;
                    } else {
                        mediaUrl = `<p><img src="${sms.media_url}" alt=""/></p>`;
                    }
                }
                ...
                let chatHtml = `<div class="chat ${sms.send_by === 'to' ? 'chat-left' : ''}">
        ...
        error: function (reject) {
            ...
            // Call the function to fetch updated messages when the page loads
            fetchUpdatedMessages();

            // Call the function to fetch updated messages every 10 seconds
            setInterval(fetchUpdatedMessages, 10000);
        });
    </script>
...
$.ajax({
    url: "{{ url('/chat-box')}}" + '/' + chatBoxId + '/reply' + '?message=' + encodeURIComponent(messageValue) + '&_token=' + encodeURIComponent("{{csrf_token()}}"),
    type: "GET", // Changed type to GET
    success: function (response) {
